...
def start_mission(mission_id: int, m_status: str):
    if m_status.upper()  != "IN_PROGRESS":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Mission start status must be: 'IN_PROGRESS'"
            )
    mission = missions_manager.get_mission_by_id(mission_id)
    if not mission:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Mission ID: {mission_id} not found"
            )
    check_rules = check_status_rules(m_status, mission, mission_id)
    if check_rules != "OK":
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=check_rules
            )
    start_update = missions_manager.update_mission_status(mission_id, m_status)
    if not start_update:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Change mission Status ID: {mission_id} failed"
            )
    return {"message": f"Mission Status ID: {mission_id} Updated 'IN_PROGRESS' Successfully"}

@mission_router.put("/missions/{id}/complete")
def complete_mission(agent_id:int, mission_id: int, m_status: str):
    ...
    complete_update = missions_manager.update_mission_status(mission_id, m_status)
    if not complete_update or not complete_agent_missions:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Change mission Status ID: {mission_id} failed"
            )
    return {"message": f"Mission Status ID: {mission_id} Updated 'COMPLETED' Successfully"}

@mission_router.put("/missions/{id}/fail")
def fail_mission(agent_id:int, mission_id: int, m_status: str):
    ...
    fail_update = missions_manager.update_mission_status(mission_id, m_status)
    if not fail_update or not fail_agent_missions:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Change mission Status ID: {mission_id} failed"
            )
    return {"message": f"Mission Status ID: {mission_id} Updated 'FAILED' Successfully"}

@mission_router.put("/missions/{id}/cancel")
def cancel_mission(mission_id: int, m_status: str):
    ...
    cancel_update = missions_manager.update_mission_status(mission_id, m_status)
    if not cancel_update:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Change mission Status ID: {mission_id} failed"
            )
    return {"message": f"Mission Status ID: {mission_id} Updated 'CANCELLED' Successfully"}

@mission_router.put("/missions/{id}/assign/{agent_id}")
def assign_mission_to_agent(mission_id: int, agent_id: int):
    ...
    is_assign = missions_manager.assign_mission(mission_id, agent_id)
    if not is_assign:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
            detail=f"Assignment of mission ID: {mission_id} to agent ID: {agent_id} failed"
            )
    return {"message": f"Mission ID: {mission_id} was successfully assigned to Agent ID: {agent_id}"}